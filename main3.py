#!/usr/bin/env python3
"""
A tiny utility script that reads two numbers and prints their product (a * b).
...
- On invalid input/usage, prints an error to stderr and exits with code 1.
"""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="main3.py",
        description="Multiply two numbers: compute a * b and print the result.",
        usage="python main3.py <a> <b>",
        add_help=True,
    )
    parser.add_argument(
        "a",
        nargs="?",
        help="First factor (number).",
    )
    parser.add_argument(
        "b",
        nargs="?",
        help="Second factor (number).",
    )
    return parser
...
def main(argv: List[str] | None = None) -> int:
    ...
    try:
        ...
        # Print as int if it's mathematically an integer (e.g., 2 * 3 => 6).
